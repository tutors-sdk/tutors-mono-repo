...
| `GET /api/home` | a signed-in user | their progress in each course of their profile: learning objects opened out of those the course's tutors.json publishes, and the last one opened; `null` for a course whose host cannot be read (Rules 0076 to 0079) |
