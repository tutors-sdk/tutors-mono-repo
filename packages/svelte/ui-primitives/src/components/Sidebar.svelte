...
<Dialog>
  ...
  <Portal>
    ...
    <Dialog.Positioner class={`fixed inset-0 z-50 flex ${positionerJustify} z-9999`}>
      <Dialog.Content
        class={`card bg-surface-100-900 overflow-y-scroll h-screen ${width} space-y-4 p-4 opacity-0 shadow-xl transition transition-discrete ${contentTranslate} data-[state=open]:opacity-100 starting:data-[state=open]:opacity-0`}
      >
