...
<style>
  ...
  :global(.paper-drawer[data-position="left"]) { border-left: 0; border-right: 1px solid var(--ui-border); box-shadow: 12px 0 40px #00000014; }
  ...
  /* A drawer slides in from the edge it is attached to (the course navigation from the left, panels from the
     right); a centred dialog fades up instead of sliding. */
  :global(.paper-drawer[data-state="open"]) { animation: drawer-enter-right 220ms cubic-bezier(0.2, 0, 0, 1); }
  :global(.paper-drawer[data-state="open"][data-position="left"]) { animation-name: drawer-enter-left; }
  :global(.paper-drawer[data-state="open"][data-presentation="dialog"]) { animation: dialog-enter 150ms ease-out; }
  @keyframes drawer-enter-right { from { transform: translateX(100%); } to { transform: translateX(0); } }
  @keyframes drawer-enter-left { from { transform: translateX(-100%); } to { transform: translateX(0); } }
