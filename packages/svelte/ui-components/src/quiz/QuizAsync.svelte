<script lang="ts">
  import { submitResponse, getAsyncResponses, getQuizUserId } from "@tutors/quiz";
  ...
  $effect(() => {
    getAsyncResponses(quiz.id, studentId).then((responses) => {
      if (responses.length > 0) {
        ...
        submitted = true;
        for (const r of responses) selectedAnswers.set(r.questionId, r.selectedIndex);
  ...
  $effect(() => {
    if (!submitted && !isLoading) startQuestionTimer();
  });
</script>
...
{:else if submitted}
  <div class="mx-auto mt-4 max-w-3xl px-4">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-xl font-bold">{quiz.title} — Results</h2>
      <button
    ...
    <QuizResults {quiz} responses={existingResponses} />
  </div>
{:else}
  <div class="mx-auto mt-4 max-w-3xl px-4 space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      {:else}
        <button class="px-4 py-2 rounded-lg text-sm preset-filled-primary-500" onclick={goNext}>
          Next
        </button>
      {/if}
    </div>
