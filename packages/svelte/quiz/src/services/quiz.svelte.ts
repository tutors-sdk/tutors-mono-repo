...
import type { Quiz, QuizQuestion, QuizResponse, QuizSession, QuestionAnalytics, QuizSource, QuizStatus, SessionStatus, QuizLiveStartedNotification } from "../types.ts";
...
const QUIZ_NOTIFY_CHANNEL = "tutors-quiz-notifications";
const QUIZ_NOTIFY_STORAGE_KEY = "tutors-quiz-live-notify";

export function broadcastQuizStarted(notification: QuizLiveStartedNotification): void {
  console.log("[quiz] broadcasting live-started notification", notification.sessionId);

  try {
    const bc = new BroadcastChannel(QUIZ_NOTIFY_CHANNEL);
    bc.postMessage(notification);
    setTimeout(() => bc.close(), 1000);
  } catch { /* BroadcastChannel unavailable */ }

  try {
    localStorage.setItem(QUIZ_NOTIFY_STORAGE_KEY, JSON.stringify({
      ...notification,
      _ts: Date.now()
    }));
...
export function listenForQuizNotifications(
  callback: (notification: QuizLiveStartedNotification) => void
): () => void {
  const cleanups: (() => void)[] = [];
  let lastProcessedTs = 0;

  function processNotification(data: QuizLiveStartedNotification & { _ts?: number }) {
    const ts = data._ts ?? 0;
    if (ts && ts <= lastProcessedTs) return;
    lastProcessedTs = ts || Date.now();
    console.log("[quiz] received live-started notification", data.sessionId);
    callback(data);
  }
  ...
  if (typeof window !== "undefined") {
    const onStorage = (event: StorageEvent) => {
      if (event.key !== QUIZ_NOTIFY_STORAGE_KEY || !event.newValue) return;
      try {
        const data = JSON.parse(event.newValue);
        if (data?.type === "quiz:live-started") {
  ...
  console.log("[quiz] notification listener active");
