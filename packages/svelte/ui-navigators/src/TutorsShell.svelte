<script lang="ts">
  ...
  let { children, hideNavigator = false, showConnect = true, title, titleHref, navigation }: { children: Snippet; hideNavigator?: boolean; showConnect?: boolean; title?: string; titleHref?: string; navigation?: Snippet } = $props();
...
<div class="tutors-shell" class:without-navigation={hideNavigator} style:--header-height={`${headerHeight}px`}>
  ...
  {#if !hideNavigator}
    <header class="shell-header" class:tucked bind:this={header} bind:offsetHeight={headerHeight} onfocusin={() => (tucked = false)}><MainNavigator {showConnect} {title} {titleHref} {navigation} /></header>
    <aside class="shell-navigation" aria-label={t("shell.navigation")}>{#if navigation}{@render navigation()}{:else}<CourseNavigation {showConnect} />{/if}</aside>
  {/if}
