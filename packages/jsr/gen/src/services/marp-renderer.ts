...
function buildMarpMarkdown(lo: Lo): string {
...
export function renderMarpToStaticHtml(lo: Lo): { slidesHtml: string; css: string } {
  const markdown = buildMarpMarkdown(lo);
  const { html, css } = marp.render(markdown);
  return { slidesHtml: html, css };
}
