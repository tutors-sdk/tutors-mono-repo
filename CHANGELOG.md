...
> This repository (`tutors-mono-repo`) is the successor to
...
- Student data goes through the reader's server: learning records, calendar time, sentiment, online status, presence, course-visit history and the course catalogue are saved by new `/api` routes that check the Auth.js session and use a private service_role key, and each row is stored under the session's login whatever the request names (Rules 0063, 0064) (network, persistence) (PR #TBD)
- Content locks: only an educator of the course (its `enrollment.yaml`, or `PRIVATE_TUTORS_ADMINS`) can lock or unlock it; the server checks, and answers 403 to anyone else (Rule 0065) (network) (PR #TBD)
- My time: a student sees their own time and their classmates under pseudonyms (`student-1`, ...), with no classmate's name or avatar (Rule 0066) (network) (PR #TBD)
- Whiteboards: edits are saved through the server, for signed-in students only, in a personal room only its owner can read or overwrite; an anonymous visitor is told to sign in to save (Rule 0068) (network, dom on `reader:whiteboard`) (PR #TBD)
...
- The student count comes from `get_student_count()`, which returns the number without reading any student's profile (Rule 0070) (network) (PR #TBD)
- The catalogue no longer deletes courses from the browser; the unused `pruneCatalogue` and `deleteCourses` are removed (PR #TBD)
...
- Monorepo migration: all packages consolidated into a single workspace repository
- Three-package UI architecture: `ui-primitives` → `ui-navigators` → `ui-components` (one-directional dependency)
- Navigator visibility now controlled via `hideNavigator` prop on `TutorsShell` rather than the `hideMainNavigator` rune (removed)
- Connect menu visibility now controlled via `showConnect` prop on `TutorsShell`

### Features

- Monorepo workspace architecture with layered package dependency graph
- `@tutors/ui-navigators` package: MainNavigator, SecondaryNavigator, Footer, layout switchers extracted from ui-components
- Route-based navigator hiding derived from `page.route.id` for lab, note, and tutorial routes
- Configurable connect menu: live and catalogue apps hide authentication/presence UI
- Tailwind CSS v4 `@source` directive for cross-package class scanning
- Comprehensive testing framework: 1323 tests across unit, BDD, component, contract, and fuzz tiers
- Mutation testing infrastructure with Stryker
- Testing guides and strategy documentation

### Upgrades

- Vite 6 → 8
- @sveltejs/vite-plugin-svelte 5 → 7
- @sveltejs/adapter-auto 3 → 7
- TypeScript 5.9 → 6.0
- Zod 3 → 4 (updated all internal API access patterns)
- pdfjs-dist updated with matching pdf.worker.min.mjs
- js-yaml 4 → 5, archiver 7 → 8, shiki-transformer-copy-button updated

### Fixes

- Cross-module `$state` reactivity: navigator hide/show broken after ui-navigators extraction, replaced with prop-based approach
- Navigator flash on lab step changes eliminated by using stable `page.route.id` instead of `currentLo` rune
- Tailwind not generating card image height classes from workspace packages — added `@source` for themes `.ts` files
- markdown-it rolled back to v14 due to @marp-team/marpit incompatibility with v15
- Stale `.svelte-kit` cache causing build failures after dependency rollbacks
- Card image sizing now properly constrained in portrait layout

### Infrastructure

- Prefetching disabled (`data-sveltekit-preload-data="off"`)
- SSR disabled on all 14 reader routes
- Netlify configs updated for live and catalogue apps (frozen lockfile, filter builds)
- Superseded gen tests removed (894 files)
