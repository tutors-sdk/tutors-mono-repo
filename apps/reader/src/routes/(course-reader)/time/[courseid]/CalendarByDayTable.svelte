...
{#if calendarByDay}
  <section class="ui-panel">
    <h2 class="ui-section-title mb-4">{t("time.calendarByDay")}</h2>
        ...
        <tbody>
          ...
          <tr class="border-b border-[var(--ui-border)] hover:bg-[var(--ui-selected)]">
            ...
            {#each dates as date}
              ...
              <td class="py-3 px-1 text-center font-mono text-xs" style="width: 36px; min-width: 36px; max-width: 36px; {heatStyle(mins)}">
                {formatCellTime(dateMinutes)}
              </td>
            {/each}
            <td class="py-3 px-4 text-right font-mono font-semibold" style="{heatStyle(calendarByDay.totalSeconds ?? 0)}">
          ...
          {#if medianRow}
            ...
            <tr class="border-b-2 border-[var(--ui-border)] bg-[var(--ui-raised)]">
              ...
              {#each dates as date}
                ...
                <td class="py-3 px-1 text-center font-mono text-xs" style="width: 36px; min-width: 36px; max-width: 36px; {heatStyle(dateBlocks ?? 0)}">
